...
// ─── Last Order persistence (localStorage) ─────────────────────────────────
const LAST_ORDER_KEY = 'wahabi_lastOrder_';

function persistLastOrder(personId, orders) {
    const hasItems = Object.values(orders).some(item => item.quantity > 0);
    if (!hasItems) return;
    try {
        localStorage.setItem(LAST_ORDER_KEY + personId, JSON.stringify(orders));
    } catch (e) { console.warn('persistLastOrder failed', e); }
}

function loadLastOrder(personId) {
    try {
        const raw = localStorage.getItem(LAST_ORDER_KEY + personId);
    ...
    Object.keys(menuData).forEach(category => {
        menuData[category].forEach(item => {
            if (orders[item.id] && orders[item.id].quantity > 0) {
                let prefix = '';
                if (category === 'balady' || category === 'shami') prefix = 'سندوتش';
                else if (category === 'jar' || category === 'appetizer') {
                    if (!item.name.includes('علبة') && !item.name.includes('باكت') &&
                        !item.name.includes('قرص') && !item.name.includes('رغيف')) prefix = 'علبة';
                }
...
// ─── Persist & restore the full persons list (names + current orders) ───────
const PERSONS_KEY = 'wahabi_persons';

function persistPersons() {
    try {
        localStorage.setItem(PERSONS_KEY, JSON.stringify(persons));
    } catch (e) { console.warn('persistPersons failed', e); }
}
...
function loadPersons() {
    try {
        ...
        if (raw) {
            ...
            // Basic validation: must be a non-empty array with id & name
            if (Array.isArray(loaded) && loaded.length > 0 && loaded[0].id && loaded[0].name) {
                return loaded.map(p => ({
                    id: p.id,
                    name: p.name,
                    orders: p.orders || {}
    ...
    } catch (e) { console.warn('loadPersons failed', e); }
    return null; // return null = use defaults
}

// On startup: restore persons from localStorage if available
(function () {
...
function updatePaymentPreview() {
    ...
    if (isPersonsMode) {
        // Persons mode: collect from every person's orders
        persons.forEach(person => {
            Object.keys(menuData).forEach(category => {
                menuData[category].forEach(item => {
                    if (person.orders[item.id] && person.orders[item.id].quantity > 0) {
                        allItems.push({
                            ...item,
                            category,
                            state: person.orders[item.id]
                        });
                    }
                });
            });
        ...
        // Normal mode: collect from global orderState
        Object.keys(menuData).forEach(category => {
            menuData[category].forEach(item => {
                if (orderState[item.id] && orderState[item.id].quantity > 0) {
                    allItems.push({
...
function renderPersonsList() {
    ...
    container.innerHTML = persons.map(person => {
        ...
        // Build action buttons only when at least one exists
        let actionsHtml = '';
        const reviewBtn = itemCount > 0
            ? `<button class="review-order-btn" onclick="event.stopPropagation(); openReviewModal(${person.id})">👁 مراجعة</button>`
            : '';
        const lastOrderBtn = hasLastOrder
            ? `<button class="last-order-btn" onclick="event.stopPropagation(); openLastOrderModal(${person.id})">🔁 نفس الطلب</button>`
            : '';

        if (reviewBtn || lastOrderBtn) {
            actionsHtml = `<div class="card-actions">${reviewBtn}${lastOrderBtn}</div>`;
        }
...
function saveCurrentPersonOrder() {
    ...
    // Persist non-empty orders to localStorage for "repeat last order"
    persistLastOrder(currentPersonId, person.orders);

    // Persist the full persons list so current orders survive page reload
    persistPersons();
}
...
// ─── Per-Person Order Review ────────────────────────────────────────────────

/**
 * Build a formatted order-text string for a single person's orders object.
 */
function buildPersonOrderText(personName, orders) {
    let text = 'طلب ' + personName + ':\n\n';
    let hasItems = false;
    ...
    Object.keys(menuData).forEach(category => {
        menuData[category].forEach(item => {
            if (orders[item.id] && orders[item.id].quantity > 0) {
                allItems.push({ ...item, category, state: orders[item.id] });
            }
        });
    });
    ...
    allItems.forEach(item => {
        ...
        let line = (item.state.quantity + ' ' + prefix + ' ' + item.name).replace(/\s+/g, ' ').trim();
        if (item.state.hasExtras) line += ' عليه بتنجان مخلل';
        text += line + '\n';
    });

    return hasItems ? text : null;
}

/**
 * Open the review modal for one person.  Called from the 👁 button on their card.
 */
function openReviewModal(personId) {
    const person = persons.find(p => p.id === personId);
    if (!person) return;

    const text = buildPersonOrderText(person.name, person.orders);
    if (!text) return;

    document.getElementById('reviewModalTitle').textContent = 'طلب ' + person.name;
    document.getElementById('reviewOrderText').textContent = text;

    const msg = document.getElementById('reviewSuccessMsg');
    if (msg) msg.classList.remove('show');

    document.getElementById('reviewPersonModal').classList.add('show');
}
...
function closeReviewModal() {
    document.getElementById('reviewPersonModal').classList.remove('show');
}

/**
 * Copy the text shown in the review modal to the clipboard.
 */
function copyReviewOrder() {
    const text = document.getElementById('reviewOrderText').textContent;
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(showMsg).catch(() => fallbackCopyReview(text, showMsg));
    } else {
        fallbackCopyReview(text, showMsg);
    }
}

function fallbackCopyReview(text, onSuccess) {
    ...
    try {
        ...
        else alert('فشل النسخ. الرجاء نسخ النص يدوياً');
    } catch (e) {
        alert('فشل النسخ. الرجاء نسخ النص يدوياً');
    }
...
// Close review modal when clicking the backdrop
document.addEventListener('click', function (e) {
    const modal = document.getElementById('reviewPersonModal');
...
function confirmApplyLastOrder() {
    ...
    // Merge last order ON TOP of whatever is already in this person's current order
    ...
    // Stay on persons list, just refresh the cards
    renderPersonsList();
}

// Close last-order modal when clicking the backdrop
document.addEventListener('click', function (e) {
    const modal = document.getElementById('lastOrderModal');
    if (modal && e.target === modal) closeLastOrderModal();
});
